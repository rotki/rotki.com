...
func (s *Service) ServeImage(ctx context.Context, w http.ResponseWriter, r *http.Request, rawURL string) {
	...
	if hasMeta {
		...
		if meta.Filename != "" && meta.Size > 0 {
			s.logger.Debug("serving from cache", "url", normalizedURL)
			s.serveCached(w, r, meta)
			return
		}
	}
...
func (s *Service) serveCached(w http.ResponseWriter, r *http.Request, meta *Metadata) {
	f, err := s.cache.OpenImage(meta.Filename)
	if err != nil || f == nil {
		s.logger.Error("failed to open cached image", "file", meta.Filename, "error", err)
		w.Header().Set("Cache-Control", "no-cache, no-store, must-revalidate")
	...
	stat, err := f.Stat()
	if err != nil {
		s.logger.Error("failed to stat cached image", "file", meta.Filename, "error", err)
		w.Header().Set("Cache-Control", "no-cache, no-store, must-revalidate")
		http.Error(w, "Cache read error", http.StatusInternalServerError)
		return
	}
