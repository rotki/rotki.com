...
watch(discountInfo, (info) => {
  ...
  if (info && info.isValid && internalValue) {
    set(modelValue, internalValue);
  }
});

watchImmediate(modelValue, (newModel: string) => {
  set(value, newModel);
});

watchImmediate([modelValue, discountInfo, plan], async ([newModel, newDiscountInfo]) => {
  if (newModel && !newDiscountInfo) {
    await fetchDiscountInfo(newModel);
  }
});

watch(plan, async () => {
  const modelVal = get(modelValue);
