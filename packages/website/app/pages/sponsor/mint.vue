...
import { ETH_ADDRESS } from '#shared/features/sponsorship/constants';
import { SPONSORSHIP_TIERS, type TierKey } from '#shared/features/sponsorship/types';
import { findTierByKey, isTierAvailable } from '#shared/features/sponsorship/utils';
...
watchEffect(() => {
  ...
  if (visible.length > 0 && firstVisible && // If current tier is not in the visible list, select the first visible one
...
watchEffect(() => {
  ...
  if (available.length > 0 && firstAvailable &&
