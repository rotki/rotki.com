...
limitations:
  - "HTX's spot trade-history endpoint exposes roughly the last 120 days of trades; older history is not retrievable via the connector. If you need older history, fall back to CSV backfill (manual entry or an aggregator that includes HTX rows)."
...
faq:
  ...
  - q: "Why is my older trade history missing?"
    a: "HTX's spot trade-history endpoint only returns roughly the last 120 days. For older history, supplement with a CSV import via a supported aggregator or manual entry."
