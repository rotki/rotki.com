...
setup:
  - "In Coinbase, create an API key from the API keys section of your settings (newer keys are created in the Coinbase Developer Platform)."
  - "Grant View (read-only) permission only, and restrict the key to the accounts/portfolio you want rotki to see."
  - "Save the key and secret. Coinbase shows the secret (or downloadable key file) only once; copy it now."
